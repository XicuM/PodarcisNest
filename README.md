...
* **Admin Web Dashboard & Debug CLI**: Web portal and rich CLI (`podarcislab`) for provisioning users, container lifecycles, and monitoring.
...
> * **Linux (systemd)**: Automatically configures and starts the `podarcislab` system service.
...
> * **Windows (PowerShell)**: Run `python -m venv .venv`, `.venv\Scripts\pip install -e .`, and `docker build -t podarcislab-user:latest .`.
...
* `--no-docker`: Skip automatic `podarcislab-user:latest` Docker image build during setup.

---

### 2. Accessing the Web Portal

Once started (via systemd or `podarcislab run`), navigate to:
...
Activate the virtual environment or run via `.venv/bin/podarcislab`:
...
podarcislab user add bob --password mysecret --run

# Container Lifecycle Control
podarcislab user start alice       # Start user workspace container
podarcislab user stop alice        # Stop user workspace container
podarcislab user restart alice     # Restart user workspace container
podarcislab user start-all         # Start all registered user containers
podarcislab user stop-all          # Stop all user containers

# Account Maintenance
podarcislab user password alice newpassword  # Reset password
podarcislab user seed alice                  # Seed or re-sync Podarcis .agents and wiki layout
podarcislab user delete alice                # Delete user and wipe workspace

# Template Asset Synchronization
podarcislab sync-templates                   # Fetch/pull latest Podarcis main branch templates
```

### 🤖 Slack Research Agent (`@podarcis`)

PodarcisLab includes a built-in Slack agent operating in **Socket Mode** with scoped access to your shared research repository (`data/shared/`).

```bash
# Check Slack configuration and knowledge base status
podarcislab slack status

# Configure with OpenCode (default) or OpenAI-compatible server
podarcislab slack config \
  --bot-token "xoxb-..." \
  --app-token "xapp-..." \
  --provider opencode \
...
# podarcislab slack config --provider anthropic --api-key sk-ant-...

# Test knowledge retrieval locally from terminal
podarcislab slack query "Summarize recent notes from the past 7 days"

# Start Slack listener in foreground
podarcislab slack start
```
...
```
Browser (User) ──> PodarcisLab Ingress Router (:8080) ──> Authenticates & Proxies
                                                                │
                ┌───────────────────────────────────────────────┴───────────────────────────────────────────────┐
                ▼                                                                                               ▼
Container: `podarcislab-user-alice` (:9003)                                     Container: `podarcislab-user-bob` (:9004)
